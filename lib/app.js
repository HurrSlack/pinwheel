...
module.exports = async function createApp(config, env = process.env) {
   ...
   * @param {PoastArgs} data
   */
  // async function supposedToBeHere({ context, payload, client, logger, next }) {
  ...
  slack.event("member_joined_channel", async function gtfoIfNecessary(data) {
    data.logger.log(data.context);
  });
