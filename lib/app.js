...
module.exports = async function createApp(config, env = process.env) {
  ...
  async function getDenyList({ client, logger }) {
    if (!cachedDenyMap) {
      ...
      const deny = (channel) => {
        logger.debug(`Denying channel ${channel.id}: #${channel.name}`);
  ...
  async function gtfoDeniedChannel(channel, { client, logger, say }) {
    logger.debug(`checking if ${channel} is in deny list`);
    try {
      ...
      if (deniedChannel) {
        ...
        return `Channel ${deniedChannel.name} has or used to have a name on the deny list`;
      }
      logger.debug(`channel ${channel} is not in deny list`);
    } catch (e) {
      return `Could not determine whether channel allows tweets: ${e}`;
    }
  }
  ...
  async function supposedToBeHere(data) {
    const deniedReason = await gtfoDeniedChannel(
      data.payload.item.channel,
      data
    );
    if (deniedReason) {
      data.logger.error(`We're not supposed to be here: ${deniedReason}`);
    } else {
      await data.next();
    }
  }
  ...
  slack.event("member_joined_channel", async (data) => {
    const { logger, payload, context } = data;
    logger.info(
      `Joined channel ${payload.channel}, checking if it's forbidden`
    );
    if (payload.user === context.botUserId) {
      await gtfoDeniedChannel(payload.channel, data);
    }
  });
