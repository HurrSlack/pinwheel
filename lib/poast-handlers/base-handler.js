...
class BasePoastHandler {
  /**
   *
   * @param {object} globalConfig - the whole kit and caboodle
   * @param {SlackItem} payload
   * @param {SlackClient} client
   * @param {Logger} logger
   */
  constructor(globalConfig, payload, client, logger) {
  ...
  getSlackDescriptor() {
    throw new Error(
      `${this.constructor.name} has not implemented getPoastDescriptor!!!`
    );
  }
}
